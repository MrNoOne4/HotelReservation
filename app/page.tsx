...
//     async function sendUserForgotOtp(data: { email: string }) {
//     const otp = generateOTP();
//     const duration = 2 * 60 * 1000;
...
//     localStorage.setItem("localForgotSaveOtp", JSON.stringify({ date: Date.now() + duration, otp: Number(otp) }));

//     showOtpForm(false);
//     setResendOTPBtn(true);
//     setCountDown(duration);
//     setResendOTPBtn(true);
//     success = await forgotOTP(data.email, otp);
//   }
  
//     const forgotOTP = async (email: string, otp: string) => {
...
//                 <ProfileMenu avatar={<AvatarWithBadge avatar={`${session?.user?.image}`} className='text-black' name={session?.user?.name?.split(",")[0]}/>} profile={() => redirect("/profile")} logout={() => signOut({ callbackUrl: window.location.href })}/>
//               </span>
//           :
...
//                   }}
//                   switchTo={() => { showSignUp(true) }} closeForm={() => { setLoginShowForm(false), showSignUp(false) }}
...
//                       closeForm={() => { setLoginShowForm(false); showSignUp(false); }} switchTo={() => { showSignUp(false) }}
//                     />
//                     :
...
//       {formForgotPassword && (
//         <div className={`w-full h-screen flex justify-center items-center fixed inset-0 bg-black/50 backdrop-blur-xs z-999999`}>
//           <ResetPasswordForm  
//           onSubmit={(e, data) => {
//              e.preventDefault();
//               setEmail(data);
...
//           submit={() => sendUserForgotOtp()};
          

//           closeForm={() => {setFormForgotPassword(false)}} switchTo={() => {setFormForgotPassword(false), setLoginShowForm(true)}}/>
//         </div>
//       )}
//     </div>
//   )
...
import { DropdownMenuIcons } from "@/components/DropdownMenuIcons";
import { AvatarWithBadge } from "@/components/AvatarWithBadge";
// FIX 3: Use useRouter instead of redirect() in a client component
...
const HotelReservation = () => {
  const [topButton, setTopButton] = useState<boolean>(false);
  const { data: session, status } = useSession();
  // FIX 3: useRouter for client-side navigation
  ...
  // Tracks which step of the forgot-password flow we are in:
  //   "email"    → user enters their email address
  //   "otp"      → user enters the OTP that was sent to their email
  //   "password" → user sets their new password
  ...
  // ─── Forgot Password: Step 1 submit — validate email exists, then send OTP ───
  ...
  // ─── Forgot Password: Step 2 submit — verify OTP ─────────────────────────────
  const handleForgotOTPSubmit = (enteredOtp: string) => {
    ...
    // OTP is valid — clear timer and move to password step
  ...
  // ─── Forgot Password: Step 3 submit — save new password ──────────────────────
  ...
  const handleClick = (element: Rooms) => {
    if (!session) {
      ...
      // FIX 3: Use router.push() instead of redirect() in a client component
  ...
  const sendMessages = async (e: React.FormEvent<HTMLFormElement>, data: messageProps) => {
    ...
    try {
      ...
      setMessage({ firstName: '', lastName: '', email: '', message: '' });
      // FIX 2: Removed the erroneous duplicate setSpinAnimation(true) that was here
      toast.success("Message has been successfully sent!", { position: "top-center" });
      ...
      // FIX 2: This correctly turns off the spinner in all cases (success, error, exception)
      setSpinAnimation(false);
    }
  };
...
// ─── Inline New Password Form component ──────────────────────────────────────
// A lightweight form used only in Step 3 of the forgot-password flow.
// Kept in this file to avoid creating a separate component file, but can
// easily be extracted to @/components/NewPasswordForm if preferred.
